...
public class ShopCartService {
   public static void dispathServiceType(String serviceType, String commodityId, MyShopCartDate myShopCartDate){  // serviceType 用于区分不同的购物车操作类型
      switch (serviceType){
         case "addToCart":
         ...
         case "updateAllCheckedBox":
            boolean isChecked = Boolean.parseBoolean(commodityId); // 将 commodityId 参数复用为全选状态
            updateAllCheckedBox(myShopCartDate, isChecked);
         break;
         case "remove":
            remove(commodityId, myShopCartDate);
            break;
         default:             // 如果没有匹配到case则不执行
            break;
      }
   }
   ...
   public static void addToCart(String commodityId, MyShopCartDate myShopCartDate){
      if(myShopCartDate.getCartList().containsKey(commodityId)){     // 当前购物车中存在该商品，则数量加一，选中状态跟随当前全选
         // 先把当前项保存下来，需要使用数量
         MyShopCartItemData myShopCartItemData = myShopCartDate.getCartList().get(commodityId);
         myShopCartDate.remove(commodityId);       // remove已经实现了更新总价
         myShopCartDate.add(myShopCartItemData.getCommodity(),myShopCartItemData.getNum()+1);
      }else{
         // 先准备 Commodity 对象，通过 commodityDao 查询商品信息
